...
public class OptionsFrame extends JDialog {
	...
	public OptionsFrame() {
		...
		textField_1.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent arg0) {
				try {
					dataStore.setMachineNumber(Integer.parseInt(textField_1
							.getText()));
					updateRuleGUI();
		...
		opoznieniaTable.setModel(new DefaultTableModel(new Object[][] {
		...
		ruleComboBox.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) {
				...
				if (ruleList.getSelectedIndex() != -1) {
					...
					oldRules[ruleList.getSelectedIndex()] = (Rule) ruleComboBox
							.getSelectedItem();
				}
				dataStore.setRules(oldRules);
			}
		});
		ruleComboBox.setModel(new DefaultComboBoxModel(new Rule[] { Rule.FIFO,
				Rule.LIFO, Rule.EDD, Rule.LPT, Rule.LWR, Rule.SPT }));
		...
		AbstractAction action2 = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					tempArr[tcl.getRow()][tcl.getColumn()] = (Integer) tcl
							.getNewValue();
	...
	public void updateRuleGUI() {
		...
		int m = 0;
		for (; m < dataStore.getRules().length
				&& m < dataStore.getMachineNumber(); m++) {
			trule[m] = oldRule[m];
		}
	...
	public void updateMacierzPrzezbrojen() {
		String[][] tempOpoznieniaArray = new String[dataStore
				.getMachineNumber()][dataStore.getJobNumber()];
		Integer[][] oldPrzezbrojenia = dataStore.getPrepareTimes();
		Integer[][] newPrzezbrojenia = new Integer[dataStore.getMachineNumber()][dataStore
				.getJobNumber()];
		...
		for (int n = 0; n < oldPrzezbrojenia.length
				&& n < dataStore.getMachineNumber(); n++)
			for (int m = 0; m < oldPrzezbrojenia[n].length
					&& m < dataStore.getJobNumber(); m++) {
		...
		table_1.setModel(new DefaultTableModel(tempOpoznieniaArray,
	...
	public void updateOpoznienieGUI() {
		...
		opoznieniaTable.setModel(new DefaultTableModel(tempOpoznieniaArray,
			...
			boolean[] rowEditables = new boolean[] { false, true };
	...
	public void updateKolejnoscGUI(int ktoryJob) {
		String[][] tempKolejnoscArray = new String[2][dataStore
				.getMachineNumber() + 1];
		String[][] oldKolejnosc = dataStore.getOrders();
		String[][] newKolejnosc = new String[dataStore.getJobNumber()][dataStore
				.getMachineNumber()];
		...
		if (ktoryJob < oldKolejnosc.length)
			for (; m < oldKolejnosc[ktoryJob].length
					&& m < dataStore.getMachineNumber(); m++) {
		...
		table.setModel(new DefaultTableModel(tempKolejnoscArray,
			...
			public boolean isCellEditable(int row, int column) {
				return column < columnEditables.length ? columnEditables[column]
						: true;
			}
		});
