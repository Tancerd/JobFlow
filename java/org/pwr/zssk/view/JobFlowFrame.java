...
public class JobFlowFrame extends JFrame {
	...
	protected void saveSimulationToPDFAction()
	{
		...
		try
		{
			PDFWriter pdfWriter = new PDFWriter(path, facade.getDataStore(), facade.getResultStore());
			pdfWriter.write();
		}
		catch (Exception e)
		{
			JOptionPane.showMessageDialog(this, "Pliku NIE zapisano poprawnie");
			return;
		}
	...
	protected void newSimulationAction() {
		...
		simulationPanel.simulationInit(dataStore.getMachineNumber(),
				dataStore.getJobNumber(), lineArray);
